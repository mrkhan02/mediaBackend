<html lang="en">
    <head>
        ...
        <link href="{{ url_for('static', filename='css/file1.css')}}" rel="stylesheet">
            ...
            <div>
                ...
                <main>
                    ...
                    <div class="shell shell--hidden">
                        <div class="shell__result">
                            <div class="shell__prompt"><i class="material-icons">chevron_right</i></div>
                            <pre tabindex="0" contenteditable="true" class="shell__text"></pre>
                        </div>
                    </div>
                </main>
                <div>
                    ...
                    <div class="card floating" id="delete-modal">
                        ...
                        <div class="card-action"><button aria-label="Cancel" title="Cancel" id="close-btn" class="button button--flat button--grey">Cancel</button><button aria-label="Create" title="Create" id="delete" class="button button--flat">Delete</button></div>
                    </div>
                    ...
                    <div class="card floating" id="copy-modal">
                        ...
                        <div class="card-action"><button aria-label="Cancel" title="Cancel" class="button button--flat button--grey" id="close-btn">Cancel </button><button aria-label="Copy" title="Copy" class="button button--flat" id="">  </button></div>
                    </div>
                    <div class="card floating" id="info-modal">
                        <div class="card-title"><h2>File information</h2></div>
                        <div class="card-content" id="info-data">

                        </div>
                        <div class="card-action"><button id='ok' aria-label="OK" title="OK" class="button button--flat"> OK </button></div></div>
                    <div class="overlay" style="display: none;"></div>
                </div>
            </div>
        </div>

        <input type="hidden" id="folder-names" value="">
        <input type="hidden" id="select" value="">
        <input type="hidden" id="selected-item" value="">
        <input type="hidden" id="multiple-selection" value="">
        <input type="hidden" id="folder-list-select" value="">
        <input type="hidden" id="folder-list-name" value="">
                    ...
                    if(clicks === 1) {
                        ...
                        timer = setTimeout(function() {
                            ...
                            if ($this.attr("aria-selected") == "true"){
                                ...
                                if ($("#select").val() == "true"){
                                    $('[aria-label="' + $("#selected-item").val() + '"]').removeAttr("aria-selected");
                                }else{
                                    $("#dropdown").prepend(action);
                                    $("#download-button").append(counter);
                                    $("#select").val("true");
                                }
                                $("#selected-item").val($this.attr("aria-label"));
                            }
                        ...
                        if (a){
                            ...
                            $.ajax({
                                ...
                                complete : function(data){
                                    ...
                                    $("#folder-names").val($("#folder-names").val() + name + "/");
                                    $("#links").append('<span class="chevron"><i class="material-icons">keyboard_arrow_right</i></span><a id="link" role="button" href="'+ $("#folder-names").val() +'" class="router-link-active">'+ $this.attr('aria-label') +'</a>');
                                }
                            });
                        }
                        clicks = 0;  //after action performed, reset counter
